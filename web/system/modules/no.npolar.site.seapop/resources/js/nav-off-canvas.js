...
var extraHeight = bigScreen ? 300 : 100;
...
$(document).ready(function(e) {
	...
	$('.toggle-subitems').click(function(e) {
		...
		if (submenu.hasClass('visible')) {
			...
			submenu.parents('li.subitems').parent("ul").attr('style', 'max-height:1200px !important;');
			$('#nav').attr('style', 'max-height:1200px !important;');
...
function getMaxHeightVal(jqueryElem) {
	...
	//return "max-height:"+ (jqueryElem.innerHeight() + extraHeight) + "px;";
}
...
function layItOut() {
	...
	if (bigScreen) {
		...
		//$('#nav .not-visible').each($(this)).removeClass('not-visible');
		//$('#nav .visible').each($(this).removeClass('visible');
		
		/*
		// Slide in leftside menu "on demand"
		$('#nav > ul > li.has_sub').click(function() {
			$('#leftside').toggleClass('visible');
		});
		*/
	}
	else {
